...
## How does it work?
Leader election is handled by `spindle`. Two APIs are provided, `Put()` and `Get()`. All pods can serve all `Get()` calls, while only the leader handles all the `Put()` calls.
