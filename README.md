...
It also works even on single-node/pod deployments.

<p align="center">
  <img src="./assets/hedge.png" width="560" title="hedge">
</p>

**hedge** is heavily used in [Alphaus](https://www.alphaus.cloud/) production with services that scale from single-digit nodes/pods to hundreds.
...
An experimental spill-over store (**SoS**) is also supported. It's currently used in bursty, quick load-process-discard type of data processing. A **SoS** in hedge is simply a combined memory area and a disk area between nodes/pods. For example, an node/pod can define a SoS of 1GB memory and 1GB disk. If there are 100 nodes/pods running under hedge, that's a combined storage of (1GB + 1GB) * 100. During writes and subsequent reads, hedge handles the data distribution and assembly between local RAM, disk, and between nodes/pods. It uses [Arrow](https://arrow.apache.org/) and memory mapped files as backing stores. You can check out this [blog post](https://flowerinthenight.com/blog/2024-07-24-spillover-store/) for more information.

Finally, a distributed semaphore is also provided through the `NewSemaphore()`, `[Try]Acquire()`, and `Release()` APIs.

## Prerequisites
* All nodes/pods within a cluster should be able to contact each other via TCP (address:port).
* Each hedge's node id should be set using the node/pod's cluster IP address:port. In Kubernetes, you can use [downward API](https://kubernetes.io/docs/concepts/workloads/pods/downward-api/) to get the pod's IP address, or you can use the ":port" format in which case the IP address will be resolved internally.
...
// For storage, any node/pod should be able to call op.Put(...) or op.Get(...) here.
//
// Any node/pod can call HasLock() here at any given time to know whether they are
// leader or not.
//
//   hl, _ := op.HasLock()
//   if hl {
...
//   }
//
// To send a message to the current leader, any node/pod can call op.Send(...) and
...
// retries+backoff.
//
// For broadcast, any node/pod can call op.Broadcast(...) here which will be handled
// by each node/pod's WithBroadcastHandler callback, including the caller.
//
// For distributed semaphore, any node/pod can call the following:
//
