...
An experimental linked storage is also supported. It's currently used in bursty, quick and adhoc load-process-discard type of data processing. A linked storage is simply a combined memory area and a disk area between pods. For example, a pod can define a linked storage of 1GB memory and 1GB disk. If there are 100 pods running under `hedge`, that's a combined storage of (1GB + 1GB) * 100. During writes, `hedge` handles the data distribution between local RAM, disk, and between pods.
