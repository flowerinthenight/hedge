...
## Why?
First, I wanted a fleet coordinator that can work within k8s Deployments as a library, not as an external service (like [ZooKeeper](https://zookeeper.apache.org/)). So far, our efforts in making [Raft](https://raft.github.io/) play well with bursty, frequently scaling up/down deployments as a library is not that reliable yet (though we also have an ongoing multi-[Paxos](https://en.wikipedia.org/wiki/Paxos_(computer_science))-based experiment [here](https://github.com/alphauslabs/juno)). Also, I wanted an easily-accessible storage that is a bit decoupled from the code (easier to query, edit, debug, backup, etc). We are already a heavy Spanner user, and `spindle` has been in our production for many years now: these two should be able to do it; StatefulSets or DaemonSets shouldn't be a requirement. Since then, additional features have been added, such as the `Send()` API.
...
An experimental linked storage is also supported. It's currently used in bursty, quick and adhoc load-process-discard type of data processing. A linked storage is simply a combined memory area and a disk area between nodes. For example, a node can define a linked storage of 1GB memory and 1GB disk. If there are 100 pods running under `hedge`, that's a combined storage of (1GB + 1GB) * 100. During writes, `hedge` handles the data distribution between local RAM, disk, and between pods.
